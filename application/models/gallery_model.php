...
class Gallery_model extends CI_Model {
	...
	function do_upload() {
		...
		$image_data = $this -> upload -> data();
			...
			$insert_data = array('name' => $image_data['file_name'], 'path' => $image_data['full_path'], 'thumb_path' => $image_data['file_path'] . 'thumbs/' . $image_data['file_name']);

			$this -> db -> insert('igallery', $insert_data);
			//load array to database
			//---upload url to database ends

			$config = array('source_image' => $image_data['full_path'], 'new_image' => $this -> gallery_path . '/thumbs', 'maintain_ration' => true, 'width' => 150, 'height' => 100);

			$this -> load -> library('image_lib', $config);
			$this -> image_lib -> resize();

			//Redirect is done right after upload is done to prevent duplicate uploading when you refresh the page
	...
	function get_images() {
		...
		$i = 0;
		//initialising counter to be used with the $image[] array
		foreach ($db_paths->result_array() as $row) {
			...
			$relativepath = substr($row['path'], $path);
			//substr(string_we_are_cutting, where_to_start)

			$image_url = base_url() . $relativepath;
			$thumb_url = str_replace("gallery/", "gallery/thumbs/", $image_url);
			//replace
	...
	function delete($id) {
		...
		$this -> db -> query("DELETE FROM `igallery` WHERE `id` =" . $id);
		//delete from harddrive (file itself)
